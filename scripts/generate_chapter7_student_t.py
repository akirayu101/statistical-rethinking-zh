...
def main():
    ...
    b=['<?xml version="1.0" encoding="UTF-8"?>',f'<svg xmlns="http://www.w3.org/2000/svg" width="{w}" height="{h}" viewBox="0 0 {w} {h}" role="img">','<title>薄尾分布与高影响力观测</title>','<desc>左图比较高斯分布与自由度为二的 Student-t 分布的概率密度，右图比较负对数概率密度；Student-t 分布的尾部下降得更慢。</desc>','<rect width="100%" height="100%" fill="#fff"/>']
    for j,left in enumerate(lefts):
        ...
        ylabel='概率密度' if j==0 else '负对数概率密度';b += [f'<text x="{left+pw/2}" y="525" text-anchor="middle" font-family="{FONT}" font-size="20" font-weight="700" fill="#263f86">数值</text>',f'<text x="{left-62}" y="{top+ph/2}" transform="rotate(-90 {left-62} {top+ph/2})" text-anchor="middle" font-family="{FONT}" font-size="20" font-weight="700" fill="#263f86">{ylabel}</text>']
